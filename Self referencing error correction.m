...
ylim([-1.1 1.1]);xlabel('Time(¦Ìs)');ylabel('Amp(a.u.)'); title('(a)','FontName','Times New Roman');
subplot 242;plot(t_temp * 1e6,real(IGMs_reshape1(flag,:)),'k','Linewidth',1);
xlim([min(t_temp)*1e6 max(t_temp)*1e6]);ylim([-1.1 1.1]);xlabel('Time(¦Ìs)');ylabel('Amp(a.u.)'); title('(b)','FontName','Times New Roman');
subplot 243;plot(t_temp * 1e6,real(IGMs_reshape2(flag,:)),'k','Linewidth',1);
xlim([min(t_temp)*1e6 max(t_temp)*1e6]);ylim([-1.1 1.1]);xlabel('Time(¦Ìs)');ylabel('Amp(a.u.)'); title('(c)','FontName','Times New Roman');
subplot 244;plot(t_temp * 1e6,real(IGMs_reshape3(flag,:)),'k','Linewidth',1);
xlim([min(t_temp)*1e6 max(t_temp)*1e6]);ylim([-1 1]);xlabel('Time(¦Ìs)');ylabel('Amp(a.u.)'); title('(d)','FontName','Times New Roman');
